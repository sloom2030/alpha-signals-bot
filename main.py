...
async def get_klines(session, symbol, interval="1h", limit=4):
    try:
...
async def analyze_candles(session, symbol):
    """تحليل الشموع لاكتشاف الزخم المبكر"""
...
def estimate_potential(change_1h, change_4h, vol_surge, is_breakout):
    """تقدير الإمكانية بناءً على الزخم الحالي"""
    score = 0
    target_low = 0
    target_high = 0
    ...
    return score, target_low, target_high, strength

# ============ الفلترة والتحليل ============
def is_blacklisted(symbol):
    return any(bl in symbol for bl in BLACKLIST)

def is_suspicious(change_24h, volume):
    if change_24h > MAX_PRICE_CHANGE_24H:
        return True, "ارتفاع مشبوه +60% في 24h"
    if volume < MIN_VOLUME_USDT:
        return True, "سيولة منخفضة جداً"
    return False, ""
...
async def scan_market(session, bot):
    ...
    for ticker in usdt_pairs:
        try:
            ...
            suspicious, reason = is_suspicious(change_24h, volume)
            if suspicious:
                continue
